...
class Scheduler():
    ...
    def __init__(self, scheduler_type='linear', lr=0.001, M=320000,
        ...
        elif self.scheduler_type == 'SE_cosineAnnealing':
            self.scheduler_function = self.SE_cosineAnnealing_scheduler
        else:
    ...
    def SE_cosineAnnealing_scheduler(self, i):
        cos_inner = np.pi * (i % (self.n_epochs // self.SE_M))
        cos_inner /= self.n_epochs // self.SE_M
        cos_out = np.cos(cos_inner) + 1
        return float(self.lr / 2 * cos_out)
...
class SE_SaveModels(Callback):
    ...
    # Arguments:
        n_epochs: total number of epochs that the model will be trained for.
        n_models: number of times the weights of the model will be saved.
        save_path: path for the filename of the weights.
    """
    ## Constructor. Args: T=250      M=5
    def __init__(self, n_epochs, n_models, save_path):
        super(SE_SaveModels, self).__init__()

        self.check = n_epochs // n_models ## 50
        self.save_path = save_path

    ## we override the method on_epoch_end of the class Callback, that
    ## is called at the end of every epoch
    def on_epoch_end(self, epoch, logs={}):
        if epoch != 0 and (epoch + 1) % self.check == 0:
            weights_file = self.save_path + '/Model%d_epoch%d.hdf5' % ((epoch + 1) // self.check, (epoch + 1))
            self.model.save_weights(weights_file, overwrite=True)
            print('Saved snapshot at %s' % weights_file)
