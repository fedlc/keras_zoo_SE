...
class Callbacks_Factory():
    ...
    def make(self, cf, valid_gen):
        ...
        if cf.SE_enabled and cf.train_model:
            ...
            scheduler = Scheduler(scheduler_type='SE_cosineAnnealing', lr=cf.learning_rate,
                                  n_epochs=cf.n_epochs, SE_n_models=cf.SE_n_models)

            cb += [LearningRateScheduler(scheduler.scheduler_function)]

            #save models
            # we create the callback calling the class we
            # create in callbacks.py, just to save the models
